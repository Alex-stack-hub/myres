...
DUMP_ENABLED = os.getenv("VLLM_NPU_DUMP", "0") == "1"
DUMP_PATH = os.getenv("VLLM_NPU_DUMP_PATH", "./npu_dump")

if DUMP_ENABLED:
    os.makedirs(DUMP_PATH, exist_ok=True)
    print(f"[WARNING] NPU Dump Enabled. Saving to: {DUMP_PATH}")

def dump_tensor(tensor: torch.Tensor, name: str, layer_idx: int = -1):
    if not DUMP_ENABLED or tensor is None:
        return
    cpu_tensor = tensor.detach().cpu().to(torch.float32)
    file_name = f"layer_{layer_idx}_{name}.npy" if layer_idx >= 0 else f"{name}.npy"
    np.save(os.path.join(DUMP_PATH, file_name), cpu_tensor.numpy())
# ==================== NPU Dump 工具代码结束 ====================
...
class Gemma4MLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor, layer_idx: int = -1) -> torch.Tensor:
        # >>> Dump: MLP 输入
        dump_tensor(x, "mlp_input", layer_idx)
        ...
        # <<< Dump: MLP 输出
        dump_tensor(x, "mlp_output", layer_idx)
        return x
...
class Gemma4Attention(nn.Module):
    def __init__(
        ...
        self.layer_idx = layer_idx  # <--- 新增这行，保存 layer_idx
    ...
    def forward(
        ...
        **kwargs,
        ...
        # >>> Dump: Attention 输入
        dump_tensor(hidden_states, "attn_input", self.layer_idx)
        ...
        # <<< Dump: Attention 输出
        dump_tensor(output, "attn_output", self.layer_idx)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.enable_moe_block:
            ...
            # >>> Dump: MoE Router 输出 (Logits)
            dump_tensor(router_logits, "moe_router_logits", self.layer_idx)
...
class Gemma4ForCausalLM(
    ...
    def compute_logits(
        ...
        # >>> Dump: 最终 Logits (用于对比最终结果)
        dump_tensor(hidden_states, "final_hidden_states")
        dump_tensor(logits, "final_logits")
