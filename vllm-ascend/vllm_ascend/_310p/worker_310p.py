...
#
# 文件功能总结：
# Ascend 310P 专用工作器 (Worker)
# 继承自 NPUWorker，提供 310P 特定的工作器实现
# 主要功能：
# 1. 设备初始化 (init_device)
# 2. 模型状态分片保存 (save_sharded_state)
# 3. 310P 特定的工作器配置
#
import torch
import torch_npu
...
class NPUWorker310(NPUWorker):
    ...
    def init_device(self):
        ...
        功能:
            1. 调用父类方法初始化设备
            2. 设置 NPU 编译模式为非 JIT 编译
            3. 初始化工作空间管理器
            4. 创建 310P 模型运行器
        """
        self.device = self._init_device()  # 调用父类方法初始化设备
        torch_npu.npu.set_compile_mode(jit_compile=False)  # 设置 NPU 编译模式为非 JIT 编译

        init_workspace_manager(self.device, num_ubatches=1)  # 初始化工作空间管理器

        self.model_runner = NPUModelRunner310(self.vllm_config, self.device)  # 创建 310P 模型运行器
